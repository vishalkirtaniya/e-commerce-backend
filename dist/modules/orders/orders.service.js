...
async function getOrderById(userId, orderNumber) {
    // 1. Main order row
    const orderResult = await db_1.default.query(`SELECT
      o.id,
      o.order_number,
      o.status,
      o.subtotal,
    ...
    WHERE o.order_number = $1 AND o.user_id = $2`, [orderNumber, userId]);
    ...
    FROM order_items
    WHERE order_id = $1
    ORDER BY id ASC`, [order.id]);
    // 3. Status history (progress tracker)
    const historyResult = await db_1.default.query(`SELECT status, label, event_date AS date
     FROM order_status_history
     WHERE order_id = $1
     ORDER BY event_date ASC`, [order.id]);
...
async function placeOrder(userId, input) {
    const { email, phone, first_name, last_name, street_address, city, state, zip_code, payment_method, } = input;
    const client = await db_1.default.connect();
    try {
        await client.query("BEGIN");
        // 1. Get user's cart
        const cartResult = await client.query(`SELECT c.id, c.promo_id, pc.discount_percent
       FROM carts c
      ...
      WHERE ci.cart_id = $1`, [cart.id]);
        ...
        // 3. Compute totals
        const subtotal = itemsResult.rows.reduce((sum, item) => sum + parseFloat(item.unit_price) * item.quantity, 0);
        ...
        const addressResult = await client.query(`INSERT INTO addresses
       ...
       VALUES ($1, 'PLACED', 'Order Placed')`, [orderId]);
        // 8. Clear the cart
